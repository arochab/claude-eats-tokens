"""
push_usage.py — moteur d'agrégation + push (tourne sur le poste local).
...
La logique de calcul PURE vit dans `usage_core` (testée). Ici on ne fait que de
...
Ce module est désormais packagé (claude_eats.push_usage). Les imports sont
relatifs (from . import usage_core), et l'écriture du repli local va dans le
dossier de config utilisateur (~/.config/claude-eats/usage.json) et non plus
`../../data`, qui n'existe pas une fois le moteur installé.

La boucle CLI complète (intervalle + refresh périodique des fenêtres officielles)
vit dans `claude_eats.cli`. `main()` ici reste un point d'entrée minimal pour la
rétrocompat des scripts .bat qui appellent `python tools/push_usage.py --once`.
...
from . import usage_core as uc
from . import statusline_windows as _sw
...
# Source A : fichier relais écrit par statusline_windows (recommandé, zéro
# réseau). Source B : endpoint OAuth /api/oauth/usage (best-effort, si le jeton
# local est encore frais). Tout est best-effort : aucune erreur ne bloque le
# push ; en l'absence de données, le front retombe sur l'estimation.
# --------------------------------------------------------------------------
...
def fetch_oauth_windows():
    ...
    try:
        ...
        # pur de statusline_windows pour normaliser (utilization/used_percentage,
        # garde-fous). Plus de spec_from_file_location : c'est un vrai module.
        win = _sw.extract_windows({"rate_limits": body})
        if win:
            win["source"] = "oauth"
        return win
    except Exception:
...
def run_cycle(url, secret=None, api_key=None, out=None, verbose=False,
    ...
    return consecutive_errors, ok


def main(argv=None):
    """Point d'entrée minimal (rétrocompat des .bat : `python tools/push_usage.py`).

    La boucle CLI riche (intervalle + refresh périodique) vit dans
    claude_eats.cli:main. Ici on garde le comportement historique : --once ou
    boucle simple, en résolvant identifiants + repli local via `config`."""
    ...
    if not url or (not secret and not api_key):
        print("⚠  Définis PUSH_URL et PUSH_SECRET (ou CET_API_KEY). "
              f"On écrit quand même {cfg.usage_path()} en local.")
